...
from Stage_2_Verifier_GPU import verify, build_verify_context
# Use Stage 1's hybrid retrieval so Stage 2 sees identical passages and produces
# the same answer as Stage 1 — then adds verification on top of it.
# Report Section 6.3.2: "Stage 2 verifies the answer generated by Stage 1."
from Stage_1_RAG_Pipeline import retrieve_hybrid as _retrieve_s1, generate_answer as _generate_s1
...
def rag_query_stage2(query, index, embedder, passages,
    ...
    # 1. Retrieve — use Stage 1's hybrid BM25+dense retrieval so both stages
    #    see identical passages and generate the same answer.
    retrieved = _retrieve_s1(query, index, embedder, passages)
    if verbose:
        print(f"\nTop {len(retrieved)} retrieved passages:")
        for i, p in enumerate(retrieved):
            print(f"  [{i+1}] {p['title']} (score: {p['score']:.4f})")

    # 2. Generate — use Stage 1's generation function (same prompt, same answer)
    if verbose:
        print("\nGenerating answer...")
    answer = _generate_s1(query, retrieved)
    if verbose:
        print(f"\nAnswer: {answer}")
    ...
    context = build_verify_context(retrieved, answer)
